...
for gearnum=1:6 %length(Catas2)
    ...
    for i=1:length(DataNames)
        ...
        for j=1:7               
            s=Vibration((j-1)*20000+1:(j-1)*20000+38400); 
...
save('E:\轴承数据\Stylegan_WT_normal.mat','x','-v7.3');             %保存时频数据

%% 生成灰度图
clc;clear;close;
load('E:\轴承数据\Stylegan_WT_normal.mat')
for i=1:length(x)  
    imshow(uint8(reshape(x(i,:,:),256,256)),'border','tight');
    colormap(flipud(gray));
    f=getframe;
    imwrite(f.cdata,['E:\轴承数据\Stylegan_normal_data\',num2str(i),'.png']); %保存的figure窗口，尺寸与分辨率不变
end